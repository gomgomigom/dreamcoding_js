...

{
  ...
  const calculate = (numbs, require) => {
    const biggerThan5 = numbs.filter((number) => number > require);
    console.log(biggerThan5);
    const result = biggerThan5.reduce((pre, cur) => pre + cur, 0);
    return result / biggerThan5.length;
  };
  console.log(calculate(numbs, 5));
}
